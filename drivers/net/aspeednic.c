...
#if defined(CONFIG_AST2300) || defined(CONFIG_AST2400)
...
unsigned int aspeednic_iobase[ASPEED_MAX_NIC] = {
  0x1E660000,
  0x1E680000
};
...
//TX
  do {
    ...
    NCSI_Request.Command = DESELECT_PACKAGE;
//Internal Channel ID = 0x1F, 0x1F means all channel
    Combined_Channel_ID = (NCSI_Cap.Package_ID << 5) + 0x1F; 
    NCSI_Request.Channel_ID = Combined_Channel_ID;
    NCSI_Request.Payload_Length = 0;
    memcpy ((unsigned char *)tx_ring[tx_new].buf, &NCSI_Request, 30);
    copy_data (NCSI_Request.Payload_Length);
...
//Internal Channel ID = 0
    Combined_Channel_ID = (NCSI_Cap.Package_ID << 5) + Channel_ID; 
    NCSI_Request.Channel_ID = Combined_Channel_ID;
    NCSI_Request.Payload_Length = 0;
    memcpy ((unsigned char *)tx_ring[tx_new].buf, &NCSI_Request, 30);
    copy_data (NCSI_Request.Payload_Length);
...
//MAC Address Num = 1 --> address filter 1, fixed in sample code
    Payload_Data[6] = 1; 
//AT + Reserved + E
    Payload_Data[7] = UNICAST + 0 + ENABLE_MAC_ADDRESS_FILTER; 
    copy_data (NCSI_Request.Payload_Length);
    aspeednic_send (dev, (void *)tx_ring[tx_new].buf, 30 + NCSI_Request.Payload_Length + 4);
//RX
    NCSI_Rx();
...
//RX
  ...
  if (NCSI_Respond.Payload_Data[3] & 0x40) {
    //Link Up or Not
    return (NCSI_Respond.Payload_Data[3] & 0x01);
  }
  else {
    //Auto Negotiate did not finish
    return 0; 
  }
}
...
//Initial State
      for (i = 0; i < 2; i++) {
      //Suppose 2 channels in current version, You could modify it to 0x1F to support 31 channels
...
static int aspeednic_recv(struct eth_device* dev)
{
  ...
  for ( ; ; )
  {
    ...
    if (status & LRS) {
      ...
      /* Ask the hardware for any other packets now that we have a spare slot */
